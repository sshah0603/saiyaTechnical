...
**Static report:** REPLACE_WITH_PAGES_URL
...
Create a codespace from the green **Code** button, Codespaces tab. The
devcontainer runs `make setup` and `make pipeline` on create, so the outputs
already exist when the terminal appears. Then:

```bash
make dashboard
```

Click **Open in Browser** on the toast, or open the **Ports** tab and click the
globe icon next to port 8501.

If `streamlit: command not found` appears, the codespace predates the
devcontainer config. Run `make setup` and try again. The `dashboard` target
invokes `python -m streamlit`, which works whether or not pip's script
directory is on PATH.

If the dashboard loads blank or keeps reconnecting, the port forwarding proxy
is blocking the websocket. Set the port to Public in the Ports tab, or add
...
**The arithmetic lives in views, not in application code.**
...
Two things that do not get solved by scaling. Subjects contribute correlated
samples, and adding more samples does not make them independent; that is a
modelling problem, not a storage one. And the five populations are
compositional, summing to 100 percent by construction, so they need a transform
...
The five populations sum to 100 percent by construction, so one rising forces
others down. Monocytes falling and CD4 T cells rising are not two independent
findings. A proper treatment applies a compositional transform, a centered
log-ratio, before testing. I left it out because it makes the output harder to
read on a first pass, and flagging it seemed better than burying it.

This is also an observational comparison inside a treated arm. A population
that separates responders from non-responders is a candidate biomarker, not a
mechanism. Confirming either point needs a prespecified analysis on a held-out
cohort.
...
`cell-count.csv` in this repo is generated by `make_sample_csv.py`, not a real
...
the production export and every script runs unchanged. The generated data
produces perfect separation on two populations, which real immunophenotyping
will not.
